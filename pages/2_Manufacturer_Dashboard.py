...
if (st.session_state["authentication_status"]) and ('authentication_status' in st.session_state):
    ...
    if st.session_state['role'] in ['manufacturer','avnet']:
        ...
        #mpns that can be forecasted
        # value_counts = df['Mock MPN'].value_counts()
        # mpns_high_data_points = value_counts.loc[value_counts > 20].index.tolist()
        ...
        if forecast_button=='Yes':
            ...
            if st.sidebar.button("Predict"):
                ...
                # result['upper_ci'] = result['Prediction']+1.645*rmse
                # result['lower_ci'] = result['Prediction']-1.645*rmse
                ...
                line=dict(width=2, color='white')
                    ...
                    # forecast_df['Order Quantity'] = np.where(forecast_df['Prediction'].isna(),forecast_df['Order Qty'],forecast_df['Prediction'])
                    # forecast_df['Prediction?'] = np.where(forecast_df['Prediction'].isna(),'No','Yes')
                    forecast_df = forecast_df[(~forecast_df['Prediction'].isna()) & (forecast_df['Order Qty'].isna())]
                    # forecast_df[['upper_ci']] = forecast_df[['upper_ci']].astype(int)
                    # forecast_df[['lower_ci']] = forecast_df[['lower_ci']].astype(int)
                    ...
                    # forecast_df = forecast_df.rename(columns ={'Prediction':'Predicted Order Quantity'})
                    
                    # st.dataframe(forecast_df.style.applymap(color_fulfilment, subset=['Prediction?']))
                    # if not os.path.exists("images"):
                    #     os.mkdir("images")
                    # fig.write_image("inages/mfg.png")
