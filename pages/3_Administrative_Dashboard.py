...
def plotly_bar_chart(
    ...
    # df = df[[x_axis,'lead _time_in months',y_axis]]
    # df = df.groupby([x_axis,'lead _time_in months']).agg(y_axis_1=(y_axis, 'sum')).sort_values([2], ascending = False).head(5).reset_index()
    ...
    if st.session_state['role'] =='avnet':
        ...
        with st.container():
            ...
            if choice=="Manufacturer":
                ...
                if group_by=='Yearly':
                    df_agg = df_manu.groupby(['Mock Manufacturer','Mock MPN','lead _time_in months','year']).agg(order_qty=('Order Qty', 'sum'), 
                               total_resale_amount=('revenue', 'sum'),backlog_count = ('Remaining qty', 'sum')).reset_index()
                else:   
                ...
                df_agg2 = df_agg.copy()
                df_agg2[['order_qty', 'total_resale_amount', 'backlog_count']] = df_agg2[['order_qty', 'total_resale_amount', 'backlog_count']].applymap('{:,.0f}'.format)
                ...
                if len(df3) ==1:
                    ...
                    kpi = st.sidebar.selectbox('Select the Feature to view',['total_resale_amount','backlog_count','order_qty'])
                    
                    
                    
                    # val = st.sidebar.selectbox('Select columns to view charts',['Lead time per MPN','MPN revenue per Manufacturer'])
                    # if val=='Lead time per MPN':
                    st.write("MPN revenue for inividual manufacturer")   
                    ...
                    if chart_type=='MPNS with Lead time information':
                        ...
                        df_agg = df_agg.groupby(['Mock Manufacturer','time']).agg( total_resale_amount=('total_resale_amount', 'sum'),backlog_count = ('backlog_count', 'sum'),order_qty = ('order_qty', 'sum')).reset_index()
                        layout = go.Layout(
            ...
            if choice=="Customer":
                ...
                df_cust_agg2[['order_qty', 'total_resale_amount', 'backlog_count']] = df_cust_agg2[['order_qty', 'total_resale_amount', 'backlog_count']].applymap('{:,.0f}'.format)
                ...
                if len(df3)==1:
                    ...
                    df_agg_cust_2 = df_cust_agg.groupby(['End Customer Number','time']).agg( total_resale_amount=('total_resale_amount', 'sum'),backlog_count = ('backlog_count', 'sum'),order_qty = ('order_qty', 'sum')).reset_index()
                    kpi = st.sidebar.selectbox('Select the Feature to view',['total_resale_amount','backlog_count','order_qty'])
                    layout = go.Layout(
                        title="Historic chart value",
                        xaxis_title="time",
                ...
                # time_frame = st.sidebar.selectbox('Select timeframe to view KPIs',['Yearly','Quarterly','Monthly'],key=3)
